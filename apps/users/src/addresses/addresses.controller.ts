...
export class AddressesController implements AddressesServiceController {
	...
	async findOne(findOneAddressDto: FindOneAddressDto): Promise<Address> {
		return await this.addressesService.findOne(
			findOneAddressDto.id,
			findOneAddressDto.user_id,
		);
	}

	async findByIds(
		findAddressByIdsDto: FindAddressByIdsDto,
	): Promise<Addresses> {
	...
	async remove(removeAddressDto: RemoveAddressDto) {
		return await this.addressesService.remove(
			removeAddressDto.id,
			removeAddressDto.user_id,
		);
	}
}
