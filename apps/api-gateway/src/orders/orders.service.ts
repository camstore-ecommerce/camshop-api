...
export class OrdersService implements OnModuleInit {
	...
	private mapOrder(order: Order): OrderDto {
		...
		return {
			...
			address: {
				...
				postal_code:
					order.address?.postal_code ?? order.user_address?.postal_code,
	...
	create(createOrderDto: CreateOrderDto): Observable<OrderDto> {
		...
		if (
			...
			createOrderDto.order_items.length === 0
		) {
			throw new BadRequestException('Order items are required');
		}

		if (
			createOrderDto.order_items.some((item) => !item.inventory_id || !item.qty)
		) {
			throw new BadRequestException('Inventory qty must be greater than 0');
		}

		createOrderDto.user_address = Struct.wrap(
			createOrderDto.user_address as any,
		) as any;
		return this.ordersServiceClient.create(createOrderDto).pipe(
	...
	findAllByUser(findAllByUser: FindAllOrderByUserDto): Observable<OrdersDto> {
		return this.ordersServiceClient.findAllByUser(findAllByUser).pipe(
			map((orders) => ({
				orders: orders.orders.map((order) => this.mapOrder(order)),
				pagination: orders.pagination,
			})),
	...
	findOneByUser(id: string, user_id: string) {
		return this.ordersServiceClient.findOneByUser({ id, user_id }).pipe(
			map((order) => this.mapOrder(order)),
			catchError((error) => {
				return throwError(() => new BadRequestException(error.message));
			}),
	...
	findOne(id: string) {
		return this.ordersServiceClient.findOne({ id }).pipe(
			map((order) => this.mapOrder(order)),
			catchError((error) => {
				return throwError(() => new BadRequestException(error.message));
			}),
