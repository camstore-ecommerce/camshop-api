...
export class AddressesService implements OnModuleInit {
	...
	constructor(
		@Inject(USERS_CLIENT) private readonly addressesClient: ClientGrpc,
	) {}

	onModuleInit() {
		this.addressesServiceClient =
			this.addressesClient.getService<AddressesServiceClient>(
				ADDRESSES_SERVICE_NAME,
			);
	}

	create(createAddressDto: CreateAddressDto, user_id: string) {
		return this.addressesServiceClient
			.create({ ...createAddressDto, user_id })
			.pipe(
				catchError((error) => {
	...
	update(id: string, updateAddressDto: UpdateAddressDto, user_id: string) {
		return this.addressesServiceClient
			.update({ ...updateAddressDto, id, user_id })
			.pipe(
				catchError((error) => {
					return throwError(() => new BadRequestException(error.message));
				}),
