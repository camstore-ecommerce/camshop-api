...
export class InventoryService {
	...
	toInventory(inventory: InventorySchema): Inventory {
		return {
			...
			...inventory,
			product: this.productsService.toProduct(
				inventory.product,
	...
	async update(_id: string, updateInventoryDto: UpdateInventoryDto) {
		...
		if (updateInventoryDto.product_id) {
			existInventory.product = await this.productsService.findOne(
		...
		return await this.inventoryRepository.findOneAndUpdate(
			{ _id },
			{ ...existInventory, ...updateInventoryDto },
		);
	}
	...
	async findAll(pagination: Pagination) {
		...
		const inventories = await this.inventoryRepository.find(
			{},
			{
				...
				sort: { [queryOptions.sort]: queryOptions.order },
			},
		);
