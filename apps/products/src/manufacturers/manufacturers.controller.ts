...
export class ManufacturersController implements ManufacturersServiceController {
	...
	async create(createManufacturerDto: CreateManufacturerDto) {
		const manufacturer: ManufacturerSchema =
			await this.manufacturersService.create(createManufacturerDto);
	...
	async findAll() {
		...
		return {
			...manufacturers,
			manufacturers: manufacturers.manufacturers.map(
	...
	async findOne(findOneManufacturerDto: FindOneManufacturerDto) {
		const manufacturer: ManufacturerSchema =
			await this.manufacturersService.findOne(findOneManufacturerDto.id);
		return this.manufacturersService.toManufacturer(manufacturer);
	}
